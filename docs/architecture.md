...
- **重疊是兩個明確的例外**：同一軌的 clip 在**畫面**上不得重疊，這條沒變。
  ...
  `dissolve_in` 讓一個 clip 的**畫面**混進前一顆的尾巴（cross dissolve）。
  ...
  溶接**結束在剪點上**而不是跨過它——它吃掉的是自己的前導，所以時間軸長度不變。
- **變速是一條貫通的鏈路**：`Clip.speed` 一改，`timeline_duration` 就變，
  `set_clip_speed` 預設 ripple 讓後面跟上。render 端畫面用 `setpts` 在 `fps` 正規化**之前**
  伸縮（之後做會把已經決定好的幀丟掉或重複），聲音用 `atempo` 且**在補長截斷之前**——
  伸縮本身就是在決定長度。`atempo` 單次只吃 0.5–2，所以超出的倍率拆成連乘。
